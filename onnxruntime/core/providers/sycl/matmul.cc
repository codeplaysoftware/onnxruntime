...
#define REGISTER_VERSIONED_MATMUL_KERNEL_TYPED(T, start, end)              \
  ONNX_OPERATOR_VERSIONED_TYPED_KERNEL_EX(                                 \
      MatMul, kOnnxDomain, start, end, T, kSyclExecutionProvider,          \
      KernelDefBuilder().TypeConstraint("T",                               \
                                        DataTypeImpl::GetTensorType<T>()), \
      MatMul<T>);

#define REGISTER_MATMUL_KERNEL_TYPED(T, start)                              \
  ONNX_OPERATOR_TYPED_KERNEL_EX(MatMul, kOnnxDomain, start, T,              \
...
Status MatMul<T>::ComputeInternal(OpKernelContext* context) const {
  ...
  if (A->Shape().NumDimensions() != B->Shape().NumDimensions()) {
    return Status(common::ONNXRUNTIME, common::NOT_IMPLEMENTED,
                  "Matmul padding/broadcasting not supported with SYCL EP");
  }
  ...
  backend.template matmul<false, false, T, int>(a_data, b_data, y_data, 0.f, M,
                                                K, N);
