...
Status Conv<T>::ComputeInternal(OpKernelContext* context) const {
  ...
  ORT_RETURN_IF_ERROR(conv_attrs_.InferOutputShape(
      input_shape, kernel_shape, strides, dilations, pads, Y_dims));
  ...
  // Allocating Intermediate Memory and Workspace Memory to perform computations
  // in NHWC format through SYCL-DNN
  ...
  snn::transpose::launch<T, Backend>(w_data, weights, weight_sizes,
                                     weight_permutations, backend);
